...
import { useNotesStore } from '../../store/stores';
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 'auto' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          {note.category && (
            <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, padding: '2px 8px', borderRadius: 'var(--radius-full)',
          ...
          <AnimatePresence>
            {showSortMenu && (
              <motion.div initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -4 }}
                style={{ position: 'absolute', right: 0, top: '100%', marginTop: 4, background: 'var(--bg-card)',
                ...
                {SORT_OPTIONS.map(opt => (
                  <button key={opt.key} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 14px',
