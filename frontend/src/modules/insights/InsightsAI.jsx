...
import { useHabitStore, useFocusStore, useFinanceStore, useConfigStore, useWorkoutStore, useReviewStore } from 'src/store/stores';
import { AreaChart, Area, XAxis, YAxis, Tooltip, ResponsiveContainer } from 'recharts';
import { generateInsights } from 'src/modules/insights/insightsEngine';
    ...
    <motion.div className="card" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
        ...
        {kpi.change !== null && kpi.change !== undefined && (
          <div style={{
            display: 'flex', alignItems: 'center', gap: 2, fontSize: 11, fontWeight: 600, fontFamily: 'var(--font-mono)',
...
function InsightCard({ item, index, variant }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, x: -16 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.05 * index }}
      style={{
        background: 'var(--bg-card)', border: '1px solid var(--border-subtle)', borderLeft: `3px solid ${c}`,
...
function PredictionCard({ item, index }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.1 * index }}
      style={{
        background: 'linear-gradient(135deg, var(--bg-card) 0%, rgba(167,139,250,0.05) 100%)', border: '1px solid var(--border-subtle)',
      ...
      <AnimatePresence>
        {isProcessing && (
          <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}
            style={{
              background: 'linear-gradient(135deg, rgba(167,139,250,0.08), rgba(255,107,107,0.05))', border: '1px solid rgba(167,139,250,0.2)',
